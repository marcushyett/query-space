...
export function GaugeChart({
  ...
  const radius = Math.min(cx - 20, gaugeType === 'semicircle' ? height - margin - 20 : (height - 40) / 2);
    ...
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative' }}>
      <svg width={width} height={height}>
        ...
        <g>
          {showValue && (
            <>
              <text x={cx} y={gaugeType === 'semicircle' ? cy - 30 : cy} textAnchor="middle" dominantBaseline="middle" fill={valueColor} fontSize={radius * 0.25} fontWeight={600}>
                {formatNumber(value)}{unit}
              </text>
              {label && (
                <text x={cx} y={gaugeType === 'semicircle' ? cy - 10 : cy + radius * 0.15} textAnchor="middle" dominantBaseline="middle" fill="#888" fontSize={12}>
                  {label}
                </text>
              )}
            </>
          ...
          {target !== undefined && (() => {
            ...
            return (
              <g>
                <circle cx={targetPos.x} cy={targetPos.y} r={6} fill="#fff" stroke="#333" strokeWidth={2} />
        ...
        <text x={polarToCartesian(cx, cy, radius + 15, gaugeParams.startAngle).x} y={polarToCartesian(cx, cy, radius + 15, gaugeParams.startAngle).y} textAnchor="middle" fill="#888" fontSize={10}>
          {formatNumber(min)}
        </text>
        <text x={polarToCartesian(cx, cy, radius + 15, gaugeParams.startAngle - gaugeParams.sweep).x} y={polarToCartesian(cx, cy, radius + 15, gaugeParams.startAngle - gaugeParams.sweep).y} textAnchor="middle" fill="#888" fontSize={10}>
          {formatNumber(max)}
        </text>
      </svg>
    </div>
