...
// Helper to generate a temporary query name from the goal
function generateQueryName(goal: string): string {
  // Truncate to first 50 chars and clean up
  const truncated = goal.slice(0, 50).trim();
  if (truncated.length < goal.length) {
    return truncated + '...';
  }
  return truncated;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Auto-create a query if projectId is provided
    if (projectId) {
      const query = await prisma.query.create({
        data: {
          projectId,
          name: generateQueryName(goal),
          description: `AI Agent session: ${goal}`,
